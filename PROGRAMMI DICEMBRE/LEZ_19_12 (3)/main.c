...
int main(void) {
	...
	while (scanf("%s", nome) != EOF) {
		...
		if (flag == 0) {  // essendoci un && nella condizione mi esce quando una delle due è falsa quindi devo capire in quale caso mi trovo
			v = (cliente*)realloc(v, (dim + 1) * sizeof(cliente)); // devo copiare lì ultimo elemento perchè io ho gia riempito il vettore fino a dim-1 (che è l'ultimo indice di posizione in quanto gli indici vanno da 0 a dim-1 cosi da avere "dim" posizioni) però c'è il tappo quindi devo allungarlo
